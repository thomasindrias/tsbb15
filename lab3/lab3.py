...
if not LAB3_IMAGE_DIRECTORY.exists():
    raise RuntimeError("Image directory '{}' does not exist. Try setting the CVL_LAB3_IMAGEDIR environment variable".format(
        LAB3_IMAGE_DIRECTORY))
...
def triangulate_optimal(C1, C2, x1, x2):
    ...
    """
    def move_orig(x): return np.array(
        [[1., 0., x[0]], [0., 1., x[1]], [0., 0., 1.]])
    T1 = move_orig(x1)
    T2 = move_orig(x2)
    ...
    def R_from_epipole(e): return np.array(
        [[e[0], e[1], 0], [-e[1], e[0], 0], [0, 0, 1]])
    ...
    # Note: Matlab implementation assumed e1, e2 homogeneous, and fk=e[-1]
    f1 = f2 = 1
    a = F[1, 1]
    b = F[1, 2]
    c = F[2, 1]
    ...
    g = [a * c * k1 * f2**4,  # coefficient for t^6
         (a**2 + c**2 * f1**2)**2 + k1 * \
         (b * c + a * d) * f2**4,  # coefficient for t^5
    ...
    def find_closest(l): return np.array([-l[0] * l[2],
                                          -l[1] * l[2],
...
def cut_out_rois(image, col_indices, row_indices, roi_size):
    ...
    m = int((roi_size - 1) / 2)
    rois = [image[r-m:r+m+1, c-m:c+m+1]
